...
public:
    bool Initialize(Command::CommandRegistry& registry, WiFiManager& manager) {
        ...
        root.Command("status")
            .Description("Show composite AP/client/scan WiFi state")
        ...
        mode.Parameter("mode", Command::ParameterKind::Enumeration)
            .OneOf({"disabled","client","ap","ap-client"});
        ...
        scan.Command("results")
            .Description("Show the most recent completed scan")
...
private:
    void ConfigureClientCommands(Command::CommandNode& client) {
        ...
        client.Command("connect").OnExecute([this](const Command::CommandContext&) {
            return Result(_manager->ConnectClient());
        });
        client.Command("disconnect").OnExecute([this](const Command::CommandContext&) {
            return Result(_manager->DisconnectClient());
        });
        ...
        password.Description("Set client password; passwords are never returned by commands");
        ...
        addressing.OnExecute([this](const Command::CommandContext& context) {
            auto config = _manager->Configuration();
            config.Client.Addressing = context.Get<std::string>("mode") == "dhcp"
                ? AddressMode::DHCP : AddressMode::Static;
            return Result(_manager->Configure(std::move(config)));
        });
    ...
    void ConfigureAccessPointCommands(Command::CommandNode& ap) {
        ap.Command("status").OnExecute([this](const Command::CommandContext&) {
            ...
            std::ostringstream out;
            out << "state=" << APStateName(state.State)
                << " ssid=" << state.SSID
                << " channel=" << static_cast<unsigned>(state.Channel)
                << " stations=" << state.ConnectedStations
                << " ip=" << state.Network.Address.ToString();
            return Command::CommandResult::Ok(out.str());
        });
        ...
        ssid.OnExecute([this](const Command::CommandContext& context) {
            auto config = _manager->Configuration();
            config.AccessPoint.SSID = context.Get<std::string>("ssid");
        ...
        password.OnExecute([this](const Command::CommandContext& context) {
            auto config = _manager->Configuration();
            config.AccessPoint.Password = context.Get<std::string>("password");
            return Result(_manager->Configure(std::move(config)));
        });
        ...
        channel.OnExecute([this](const Command::CommandContext& context) {
            auto config = _manager->Configuration();
            config.AccessPoint.Channel = static_cast<uint8_t>(context.Get<unsigned int>("channel"));
            return Result(_manager->Configure(std::move(config)));
        });
        ...
        enabled.OnExecute([this](const Command::CommandContext& context) {
            auto config = _manager->Configuration();
            config.AccessPoint.DHCP.Enabled = context.Get<bool>("enabled");
            return Result(_manager->Configure(std::move(config)));
        });
    ...
    void ConfigureConfigurationCommands(Command::CommandNode& config) {
        config.Command("show")
            .Description("Show configuration with credentials always redacted")
            .OnExecute([this](const Command::CommandContext&) { return ShowConfiguration(); });

        config.Command("save").OnExecute([this](const Command::CommandContext&) {
            return StoreResult(_manager->SaveConfiguration());
        });

        config.Command("load").OnExecute([this](const Command::CommandContext&) {
            return StoreResult(_manager->LoadConfiguration(true));
        });

        config.Command("reset").OnExecute([this](const Command::CommandContext&) {
            return Result(_manager->Configure(WiFiConfiguration{}));
        });

        auto& hostname = config.Command("hostname");
        hostname.Parameter<std::string>("hostname");
        hostname.OnExecute([this](const Command::CommandContext& context) {
            auto value = _manager->Configuration();
            value.Hostname = context.Get<std::string>("hostname");
            return Result(_manager->Configure(std::move(value)));
        });

        auto& tx = config.Command("tx-power");
        tx.Parameter<int>("dbm").Range(2,20);
        tx.OnExecute([this](const Command::CommandContext& context) {
            auto value = _manager->Configuration();
            value.TxPowerDbm = static_cast<int8_t>(context.Get<int>("dbm"));
            return Result(_manager->Configure(std::move(value)));
        });

        auto& powerSave = config.Command("power-save");
        powerSave.Parameter<bool>("enabled");
        powerSave.OnExecute([this](const Command::CommandContext& context) {
            auto value = _manager->Configuration();
            value.PowerSave = context.Get<bool>("enabled");
            return Result(_manager->Configure(std::move(value)));
        });
    }
    ...
    void AddIPv4Setter(Command::CommandNode& parent, const char* name, Setter setter) {
        auto& command = parent.Command(name);
        command.Parameter<std::string>("address");
        command.OnExecute([this, setter](const Command::CommandContext& context) {
            IPv4Address address;
            if (!ParseIPv4(context.Get<std::string>("address"), address)) {
                return Command::CommandResult::Error("Invalid IPv4 address");
            }
            auto config = _manager->Configuration();
            setter(config, address);
            return Result(_manager->Configure(std::move(config)));
        });
    }
    ...
    Command::CommandResult Status() const {
        ...
        std::ostringstream out;
        out << "mode=" << ModeName(state.Mode)
            << " ap=" << APStateName(state.AccessPoint.State)
            << " stations=" << state.AccessPoint.ConnectedStations
            << " client=" << ClientStateName(state.Client.State)
            << " ip=" << state.Client.Network.Address.ToString()
            << " scan=" << ScanStateName(state.Scan);
        return Command::CommandResult::Ok(out.str());
    }
    ...
    Command::CommandResult ScanResults() const {
        ...
        std::ostringstream out;
        out << "networks=" << results.size();
        for (std::size_t i = 0; i < results.size(); ++i) {
            out << "\n[" << i << "] ssid=" << results[i].SSID
                << " rssi=" << results[i].RSSI
                << " channel=" << static_cast<unsigned>(results[i].Channel)
                << " security=" << SecurityName(results[i].Security);
        }
        return Command::CommandResult::Ok(out.str());
    }
    ...
    Command::CommandResult ShowConfiguration() const {
        ...
        std::ostringstream out;
        out << "mode=" << ModeName(c.Mode)
            << " hostname=" << c.Hostname
            << " tx-power=" << static_cast<int>(c.TxPowerDbm)
            << " power-save=" << (c.PowerSave ? "true" : "false")
            << "\nap.ssid=" << c.AccessPoint.SSID
            << " ap.password=<redacted>"
            << " ap.channel=" << static_cast<unsigned>(c.AccessPoint.Channel)
            << " ap.dhcp=" << (c.AccessPoint.DHCP.Enabled ? "true" : "false")
            << "\nclient.ssid=" << c.Client.SSID
            << " client.password=<redacted>"
            << " client.addressing=" << (c.Client.Addressing == AddressMode::DHCP ? "dhcp" : "static");
        return Command::CommandResult::Ok(out.str());
    }

    static Command::CommandResult Result(WiFiStatus status) {
        return status == WiFiStatus::Success
            ? Command::CommandResult::Ok("OK")
            : Command::CommandResult::Error(StatusName(status));
    }

    static Command::CommandResult StoreResult(const WiFiConfigurationStoreResult& result) {
        return result.Success()
            ? Command::CommandResult::Ok("OK")
            : Command::CommandResult::Error(result.Message.empty() ? "WiFi configuration persistence failed" : result.Message);
    }
    ...
    static bool ParseIPv4(const std::string& text, IPv4Address& output) {
        ...
        output = IPv4Address(static_cast<uint8_t>(a), static_cast<uint8_t>(b), static_cast<uint8_t>(c), static_cast<uint8_t>(d));
        return true;
    }

    static const char* StatusName(WiFiStatus value) {
        switch(value) {
            case WiFiStatus::Success:return "success";
            case WiFiStatus::InvalidConfiguration:return "invalid configuration";
            case WiFiStatus::NotSupported:return "not supported";
            case WiFiStatus::Busy:return "busy";
            default:return "platform error";
        }
    }

    static const char* ModeName(WiFiMode value) {
        switch(value) {
            case WiFiMode::Disabled:return "disabled";
            case WiFiMode::Client:return "client";
            case WiFiMode::AccessPoint:return "ap";
            default:return "ap-client";
        }
    }

    static const char* ClientStateName(ClientState value) {
        switch(value) {
    ...
    static const char* APStateName(AccessPointState value) {
        switch(value) {
            case AccessPointState::Disabled:return "disabled";
            case AccessPointState::Starting:return "starting";
            case AccessPointState::Active:return "active";
            default:return "failed";
        }
    }

    static const char* ScanStateName(ScanState value) {
        switch(value) {
    ...
    static const char* SecurityName(NetworkSecurity value) {
        switch(value) {
            case NetworkSecurity::Open:return "open";
            case NetworkSecurity::WEP:return "wep";
            case NetworkSecurity::WPA:return "wpa";
            case NetworkSecurity::WPA2:return "wpa2";
            case NetworkSecurity::WPA_WPA2:return "wpa-wpa2";
            case NetworkSecurity::WPA3:return "wpa3";
            case NetworkSecurity::WPA2_WPA3:return "wpa2-wpa3";
